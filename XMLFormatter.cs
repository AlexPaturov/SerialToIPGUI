...
namespace WFClientServer
{
    ...
    {
        private static Dictionary<string, string> ParseInputString(string rawData) 
        {
            Dictionary<string, string> XMLformated = RawToXML(rawData);
            return XMLformated;
        }

        // сюда 100% приходит не 0 длина строки
        private static Dictionary<string, string> RawToXML(string input) 
        {
            ...
            if (workInput.Contains('\n')) // просто скопировать остаток строки в 
            {
                string delta = workInput.Substring(0, workInput.IndexOf(" "));
                XMLtmp.Add("Delta", workInput.Substring(0, workInput.IndexOf(" ")));
                workInput = workInput.Substring(workInput.IndexOf(" ") + 1, (workInput.Length - (workInput.IndexOf(" ") + 1)));             // 11
            }
            else 
            {
                string delta = workInput;
        ...
        public static string getStatic(string input) 
        {
            ...
            xmlDoc.AppendChild(rootResponse);                                                           // Append the root element to the document
                ...
                ch1_CheckSumWeight.InnerText = "54647";
